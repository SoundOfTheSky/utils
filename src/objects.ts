...
export function getPropertyNames(e: object, keys = new Set()) {
  ...
  for (let i = 0; i < own.length; i++) keys.add(own[i]);
  const proto = Object.getPrototypeOf(e) as object | undefined;
...
export const objectMap = (
  object: object,
  fn: (key: string, val: unknown) => [string, unknown],
) =>
  Object.fromEntries(Object.entries(object).map(([key, val]) => fn(key, val)));

/** Filter function like for arrays, but for objects */
export const objectFilter = (
  object: object,
  fn: (key: string, val: unknown) => unknown,
) =>
  Object.fromEntries(
    Object.entries(object).filter(([key, val]) => fn(key, val)),
  );
...
export function addPrefixToObject<
  ...
  P extends string,
>(obj: Record<string, T>, prefix: P): ObjectAddPrefix<T, P> {
...
export function deepEquals(
  ...
  if (Array.isArray(a)) {
    ...
    for (let i = 0; i < a.length; i++)
      if (!deepEquals(a[i], b[i], stack)) return false;
    return true;
  }
